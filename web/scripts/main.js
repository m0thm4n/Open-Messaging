...
let onMessage = (data) => {
  switch (data.metadata.type) {
    ...
    case "message":
      ...
      let participant = currentConversation.participants.find(
        (p) => p.chats[0].id == senderId
      )
...
function showChatTranscript(conversationId) {
  return conversationsApi
    .getConversationsChatMessages(conversationId)
    .then((data) => {
      // Show each message
      data.entities.forEach((msg) => {
        if (msg.hasOwnProperty("body")) {
          ...
          let name = currentConversation.participants.find(
            (p) => p.chats[0].id == senderId
          ).name
          let purpose = currentConversation.participants.find(
            (p) => p.chats[0].id == senderId
          ).purpose
